...
function tree(schema, options) {
  ...
  schema.pre('save', function(next) {
    ...
    if(this.isNew || isParentChange) {
      ...
      this.collection.findOne({ _id : mongoose.Types.ObjectId(this.parent) }, function(err, doc) {
        ...
        if(!doc) console.log(self.login, self.parent)
  ...
  var getDescendantsCount = function(cb) {
    var filter = { path : { $regex : '^' + this.path + pathSeparator } }
    return this.model(this.constructor.modelName).count(filter, cb);
  };

  schema.method('getDescendantsCount', getDescendantsCount);
